 ...
 *
 * @param {{ user_id: string, session_id: string|null, message: string, lat: number|null, lng: number|null, language: string }} params
 ...
 */
export async function sendChatMessage({ user_id, session_id, message, lat, lng, language }) {
  let response;
  try {
    response = await fetch(`${BASE_URL}/api/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user_id, session_id, message, lat, lng, language }),
    });
  } catch {
